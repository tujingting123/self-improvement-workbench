...
const Store = {
  ...
  getDailyKnowledge(dbType, knowledgeDB, count = 5) {
    ...
    // 兜底：如果 todayIds 为空（可能是旧数据迁移），重新选取
    if (todayIds.length === 0) {
      ...
      this.data[key] = tracker;
      this.save();
      const items = tracker.todayIds.map(id => knowledgeDB.find(k => k.id === id)).filter(Boolean);
      return { items, cycle: tracker.cycle || 0, total: knowledgeDB.length, read: (tracker.readIds || []).length + tracker.todayIds.length };
    }
